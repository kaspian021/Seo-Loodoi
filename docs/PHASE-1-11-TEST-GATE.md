...
- **E2E AI journey** (`workspace.spec.ts`, no route/API mocks): register → one project on the fixture host → real crawl + durable analysis → `analysis-status` Succeeded → AI analyze returns the structured output (`provider: deterministic-expert-engine`, `promptVersion: 1.0.0`, non-empty summary/observations/rootCauses/missingEvidence) → the UI renders the new sections and badge → entitlements show plan Starter and **`aiCreditsUsed` exactly 1**.

Still open (not claimed solved): live `openai-compatible` provider acceptance with a real endpoint/key (CI only exercises the deterministic engine and a scripted fake handler); the nine-locale translation review for the new keys; load testing, live Google/vendor providers, real third-party crawls, JS rendering and measured CWV. The previously documented duplicate-host 500 was closed in the production-release slice: duplicate owner/host creation now returns HTTP 409.
...
**E2E-discovered product gap (closed 2026-09-21):** creating a second project for the same owner with an already-claimed host formerly returned HTTP 500. The production-release slice added an owner/normalized-host pre-check plus a unique-violation race guard; the API now returns HTTP 409. `Phase1To11ContractTests.ProjectCreation_DuplicateOwnerHost_ReturnsConflictInsteadOfServerError` pins the contract.
