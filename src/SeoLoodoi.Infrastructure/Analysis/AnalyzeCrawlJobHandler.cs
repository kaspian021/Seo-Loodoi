...
public sealed class AnalyzeCrawlJobHandler(AppDbContext db, IEnumerable<ISeoRule> rules, IScoringEngine scoring, IContentSimilarityEngine similarity, IInternalLinkGraph linkGraph, IUrlNormalizer normalizer, ILogger<AnalyzeCrawlJobHandler> logger) : ISeoJobHandler
{
    ...
    private static string Title(string code) => code switch
    {
        ...
        "META_ROBOTS_NOFOLLOW" => "متا ربات nofollow لینک‌های صفحه را برای موتور جستجو غیرقابلدنبال کرده است",
        "META_ROBOTS_NOARCHIVE" => "متا ربات noarchive نمایش نسخه کش‌شده صفحه را غیرفعال کرده است",
        "META_ROBOTS_NOSNIPPET" => "متا ربات nosnippet نمایش توضیح صفحه در نتایج جستجو را غیرفعال کرده است",
        "META_ROBOTS_NOIMAGEINDEX" => "متا ربات noimageindex تصاویر صفحه را از جستجوی تصویری حذف کرده است",
        "X_ROBOTS_NOFOLLOW" => "هدر X-Robots-Tag پیگیری لینک‌های صفحه را غیرفعال کرده است",
        "INDEXABILITY_CONTRADICTION" => "دستورهای نمایه‌سازی متا ربات و هدر X-Robots-Tag با هم تناقض دارند",
        "CANONICAL_EXTERNAL" => "Canonical صفحه به دامنه دیگری اشاره می‌کند",
        "SOFT_404" => "صفحه با کد 200 پاسخ می‌دهد اما محتوای صفحه پیدا نشده (Soft 404) است",
        "REDIRECT_LOOP" => "زنجیره تغییر مسیر حلقه دارد و به مقصد نمی‌رسد",
        "CONTENT_TYPE_UNSUPPORTED" => "نوع محتوای صفحه برای استخراج سئو مناسب نیست",
        "URL_TOO_LONG" => "نشانی صفحه (URL) بیش از حد طولانی است",
        "URL_TOO_DEEP" => "مسیر صفحه در URL بیش از حد عمیق است",
        "URL_UPPERCASE" => "مسیر URL حروف بزرگ دارد و خطر ایجاد نشانی‌های تکراری وجود دارد",
        "URL_ENCODING_ANOMALY" => "کدگذاری درصدی غیرعادی در URL صفحه وجود دارد",
        "URL_FRAGMENT" => "نشانی صفحه دارای بخش (Fragment) اضافه است",
        "URL_TRACKING_PARAMETER" => "پارامترهای ردیابی تحلیلی در URL صفحه وجود دارد",
        "URL_PARAMETER_DUPLICATE" => "یک پارامتر چند بار در URL صفحه تکرار شده است",
