...
internal sealed class CrawlFrontierStore(AppDbContext db) : ICrawlFrontierStore
{
    ...
    public async Task<CrawlFrontierItem?> TryLeaseAsync(Guid crawlId, string workerId, DateTimeOffset now, TimeSpan leaseDuration, CancellationToken ct)
    {
        if (!db.Database.IsRelational())
        {
            var candidate = await db.CrawlFrontierItems.Where(x => x.CrawlId == crawlId && (x.Status == FrontierStatus.Pending || (x.Status == FrontierStatus.Leased && x.LeaseExpiresAt <= now)) && (x.NotBefore == null || x.NotBefore <= now)).OrderBy(x => x.Depth).ThenBy(x => x.CreatedAt).FirstOrDefaultAsync(ct);
            if (candidate is not null && candidate.TryLease(workerId, now, leaseDuration)) await db.SaveChangesAsync(ct);
            return candidate;
        }
        await using var tx = await db.Database.BeginTransactionAsync(ct);
        var item = await db.CrawlFrontierItems
            .FromSqlInterpolated($"SELECT * FROM loodoi.\"CrawlFrontierItems\" WHERE \"CrawlId\" = {crawlId} AND (\"Status\" = 0 OR (\"Status\" = 1 AND \"LeaseExpiresAt\" <= {now})) AND (\"NotBefore\" IS NULL OR \"NotBefore\" <= {now}) ORDER BY \"Depth\", \"CreatedAt\" FOR UPDATE SKIP LOCKED LIMIT 1")
            .AsTracking().SingleOrDefaultAsync(ct);
        if (item is not null && item.TryLease(workerId, now, leaseDuration)) await db.SaveChangesAsync(ct);
...
{
    public async Task<Guid> EnqueueOnceAsync(SeoJobType type, string idempotencyKey, string payloadJson, CancellationToken ct)
    {
        var job = new SeoBackgroundJob(type, idempotencyKey, payloadJson);
        if (!db.Database.IsRelational())
        {
        ...
        await db.Database.ExecuteSqlInterpolatedAsync($"INSERT INTO loodoi.\"SeoBackgroundJobs\" (\"Id\",\"CreatedAt\",\"UpdatedAt\",\"Type\",\"IdempotencyKey\",\"PayloadJson\",\"Status\",\"Attempts\") VALUES ({job.Id},{job.CreatedAt},{job.UpdatedAt},{(int)job.Type},{job.IdempotencyKey},{job.PayloadJson},{(int)job.Status},{job.Attempts}) ON CONFLICT (\"IdempotencyKey\") DO NOTHING", ct);
    ...
    public async Task<SeoBackgroundJob?> TryLeaseAsync(string workerId, DateTimeOffset now, TimeSpan leaseDuration, CancellationToken ct)
    {
        if (!db.Database.IsRelational())
        {
            var candidate = await db.SeoBackgroundJobs.Where(x => (x.Status == SeoJobStatus.Queued || (x.Status == SeoJobStatus.Running && x.LeaseExpiresAt <= now)) && (x.NotBefore == null || x.NotBefore <= now)).OrderBy(x => x.CreatedAt).FirstOrDefaultAsync(ct);
            if (candidate is not null && candidate.TryLease(workerId, now, leaseDuration)) await db.SaveChangesAsync(ct);
            return candidate;
        }
        await using var tx = await db.Database.BeginTransactionAsync(ct);
        var job = await db.SeoBackgroundJobs
            .FromSqlInterpolated($"SELECT * FROM loodoi.\"SeoBackgroundJobs\" WHERE (\"Status\" = 0 OR (\"Status\" = 1 AND \"LeaseExpiresAt\" <= {now})) AND (\"NotBefore\" IS NULL OR \"NotBefore\" <= {now}) ORDER BY \"CreatedAt\" FOR UPDATE SKIP LOCKED LIMIT 1")
            .AsTracking().SingleOrDefaultAsync(ct);
        if (job is not null && job.TryLease(workerId, now, leaseDuration)) await db.SaveChangesAsync(ct);
