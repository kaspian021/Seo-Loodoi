...
export type TeamSeats={plan:string;limit:number;used:number;remaining:number}
...
export const api={
  ...
  suspendMember:(projectId:string,id:string,suspend:boolean)=>request<void>(`/api/seo/projects/${projectId}/members/${id}/${suspend?'suspend':'reactivate'}`,{method:'POST'}),
  teamSeats:(projectId:string)=>request<TeamSeats>(`/api/seo/projects/${projectId}/members/seats`),
  invitations:(projectId:string)=>request<ProjectInvitation[]>(`/api/seo/projects/${projectId}/invitations`),
  invite:(projectId:string,email:string,role:'Viewer'|'Editor'|'Admin')=>request<{invitation:ProjectInvitation;emailDelivered:boolean}>(`/api/seo/projects/${projectId}/invitations`,{method:'POST',body:JSON.stringify({email,role})}),
  cancelInvitation:(projectId:string,id:string)=>request<void>(`/api/seo/projects/${projectId}/invitations/${id}`,{method:'DELETE'}),
  acceptInvitation:(token:string)=>request<ProjectMember>('/api/seo/invitations/accept',{method:'POST',body:JSON.stringify({token})}),
